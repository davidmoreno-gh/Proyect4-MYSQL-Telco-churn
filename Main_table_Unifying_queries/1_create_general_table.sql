...
FROM churn_demographics as demo
LEFT JOIN churn_location as loc ON demo.`Customer ID` = loc.`Customer ID`
LEFT JOIN churn_services as ser ON demo.`Customer ID` = ser.`Customer ID`
LEFT JOIN churn_status as st ON demo.`Customer ID` = st.`Customer ID`